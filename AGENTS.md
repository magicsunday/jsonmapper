...
2. `composer ci:test:php:phpstan` — ensure no failures for changed files; project uses maximum PHPStan level.
3. `composer ci:test:php:cpd` (or `npx jscpd --config .build/.jscpd.json`) — keep the duplication check green.
4. `composer ci:test:php:unit:coverage` — ensure green status with ≥ 90% coverage.
